...
/**
 * FAMOUS オプションTools デフォルト画像設定フィルター
 */
function famous_theme_options_tools_get_default_images_settings( $default_images_settings ) {
	// デフォルト画像設定
	return array(
/*
		array(
...
*/
		array(
			'media_filename' => 'famous-op_top-slider.gif',
			'source_filepath' => get_template_directory() . '/admin/img/default/op_top-slider.gif',
			'theme_option_keys' => array( 'slider_image1', 'slider_image2', 'slider_image3' )
		),
		array(
			'media_filename' => 'image_1450x650.gif',
			'source_filepath' => get_template_directory() . '/admin/img/default/image_1450x650.gif',
			'theme_option_keys' => array( 'footer_image' )
		),
		array(
			'media_filename' => 'famous-image_300x300.gif',
			'source_filepath' => get_template_directory() . '/admin/img/default/image_300x300.gif',
			'theme_option_keys' => array( "['contents_builder'][0]['cb_image1']", "['contents_builder'][0]['cb_image2']", "['contents_builder'][0]['cb_image3']" )
		),
		array(
			'media_filename' => 'famous-op_parallax.gif',
			'source_filepath' => get_template_directory() . '/admin/img/default/op_parallax.gif',
			'theme_option_keys' => array( "['contents_builder'][1]['cb_background_image']", "['contents_builder'][3]['cb_background_image']" )
		),
		array(
			'media_filename' => 'famous-image_1180x400.gif',
			'source_filepath' => get_template_directory() . '/admin/img/default/image_1180x400.gif',
			'theme_option_keys' => array( "['contents_builder'][5]['cb_image']" )
		)
	);
}

/**
 * FAMOUS オプションTools サンプル記事
 */
function famous_theme_options_tools_set_sample_posts( $dp_options ) {
	...
	$media_ids['post'] = (int) theme_options_tools_get_media_id( array(
		'media_filename' => 'famous-image_image_830x575.gif',
		'source_filepath' => get_template_directory() . '/admin/img/default/image_830x575.gif'
	), true );
	$media_ids['works'] = (int) theme_options_tools_get_media_id( array(
		'media_filename' => 'famous-image_1180x730.gif',
		'source_filepath' => get_template_directory() . '/admin/img/default/image_1180x730.gif'
	), true );
	$media_ids['voice'] = (int) theme_options_tools_get_media_id( array(
		'media_filename' => 'famous-image_150x150.gif',
		'source_filepath' => get_template_directory() . '/admin/img/default/image_150x150.gif'
	), true );
	...
	for ( $i = 1; $i <= 4; $i++ ) {
		$sample_posts[] = array(
			'post_title' => __( 'Sample works', 'tcd-w' ) . $i,
			'post_content' => '',
			...
			// カスタムフィールド meta_key => meta_value
			'metas' => array(
				'repeater_gallery' => array(
					array(
						'media_type' => 'image',
						'title' => 'desc',
						'image' => $media_ids['works'],
	...
	for ( $i = 1; $i <= 4; $i++ ) {
		$sample_posts[] = array(
			'post_title' => __( 'Sample voice', 'tcd-w' ) . $i,
			'post_content' => '',
			'post_name' => 'sample-voice' . $i,
			'post_status' => 'publish',
			'post_type' => 'voice',
			'post_date' => $post_date_base . sprintf( '%02d', $i ),
			// アイキャッチ メディアIDもしくはtheme_options_tools_get_media_id()用の配列もしくはコピー元ファイルパス
...
/**
 * FAMOUS オプションTools サンプルメニュー
 */
function famous_theme_options_tools_set_sample_menus( $dp_options ) {
	// 新規サイト以外、リセットチェックなしの場合は終了
	...
	if ( ! empty( $menu_locations['global_front'] ) && $nav_menus && ! is_wp_error( $nav_menus ) ) {
		foreach ( $nav_menus as $nav_menu ) {
			if ( $nav_menu->term_id == $menu_locations['global_front'] )
				return $dp_options;
		}
	}
	if ( ! empty( $menu_locations['global_sub'] ) && $nav_menus && ! is_wp_error( $nav_menus ) ) {
		foreach ( $nav_menus as $nav_menu ) {
			if ( $nav_menu->term_id == $menu_locations['global_sub'] )
				return $dp_options;
		}
	}
	if ( ! empty( $menu_locations['footer'] ) && $nav_menus && ! is_wp_error( $nav_menus ) ) {
		foreach ( $nav_menus as $nav_menu ) {
			if ( $nav_menu->term_id == $menu_locations['footer'] )
				return $dp_options;
		}
	}
	...
	$menu_locations['global_front'] = (int) $menu_id;
	$menu_locations['global_sub'] = (int) $menu_id;
...
/**
 * FAMOUS オプションTools サンプルウィジェット
 */
function famous_theme_options_tools_set_sample_widgets( $dp_options ) {
	// 新規サイト以外、リセットチェックなしの場合は終了
		...
		array(
			...
			'params' => array(
				...
				'banner_image1' => (int) theme_options_tools_get_media_id( array(
					'media_filename' => 'famous-image_300x250.gif',
 ...
 * FAMOUS テーマ初期化フィルター
 */
add_filter( 'tcd_theme_options_tools-get_default_images_settings', 'famous_theme_options_tools_get_default_images_settings', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'famous_theme_options_tools_set_sample_posts', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'famous_theme_options_tools_set_sample_menus', 10, 1 );
add_filter( 'tcd_theme_options_tools-initialize', 'famous_theme_options_tools_set_sample_widgets', 10, 1 );
